...
API分为两部分：**REST API和Websocket 实时数据流**
 ...
 -  REST API包含四个类别：**[用户模块](#用户模块)（私有），[交易模块](#交易模块)（私有），[市场数据](#市场数据)（公共），其他（公共）**
...
1.  [沙盒环境](#沙盒测试环境) 帮助您在测试环境更好地了解和使用API。
2.  [REST&nbsp;API](#REST&nbsp;API) 如何创建一个REST&nbsp;API。
3.  [服务器时间](#时间) 无需身份验证的情况下，发起一个请求并获取返回结果值（可用作连接测试）。
4.  [接口认证](#接口认证) 如何创建一个有身份验证的请求。
5.  [内部资金划转](#内部资金划转) 储蓄账户和交易账户之间资产的相互划转。
6.  [账户列表](#账户列表) 获取个人的账户详情指南。
7.  [下单](#下单) 获取下单操作指南。
8.  [委托买卖盘](#委托买卖盘) 获取买卖盘的快照信息。
9.  [Websocket](#申请连接令牌) 如何创建Websocket 连接
10. [Level-2 市场行情](#Level-2市场行情) 如何使用Websocket 构建一个实时的买卖盘。 
11. [余额变更](#余额变更事件) Websocket私人频道指南，实时获取账户余额变更信息
...
- 做市商返佣
- 每季度奖励高达100,000 KCS，来回馈最大交易量的做市商
- 直接市场接入和主机托管服务
...
---------- | -------
[账户列表](#账户列表) | 获取所有账户信息
[单个账户详情](#单个账户详情) | 获取单个账户信息
[创建账户](#创建账户) | 创建账户
[账户流水记录](#账户流水记录) | 获取账户流水记录
[账户冻结记录](#账户冻结记录) | 获取账户冻结记录
[内部资金划转](#内部资金划转) | 储蓄账户和交易账户资金相互划转
[下单](#下单) | 下单
[单个撤单](#单个撤单) | 取消单个订单
[全部撤单](#全部撤单) | 取消所有订单
[订单记录](#订单记录) | 获取订单记录
[最近订单记录](#最近订单记录) | 获取最近24小时内的订单列表（最多获取1000条记录）
[单个订单详情](#单个订单详情) | 获取单个订单详情
[成交记录](#成交记录) | 获取成交记录
[最近成交记录](#最近成交记录) | 获取最近24小时内的成交记录（最多1000条记录）
...
- 添加 [申请提现](#申请提现)，[获取提现额度](#获取提现额度)，[申请充值地址](#申请充值地址)，[获取充值地址](#获取充值地址)，[币种详情](#币种详情)，添加**chain**字段。
- 添加 [内部资金划转](#内部资金划转) 添加资金划转操作步骤。
- 添加 对接[Level-3撮合引擎数据](#完整的撮合引擎数据(Level&nbsp;3))的[go-level-3-demo](#客户端开发库)
- 修改 [请求频率限制](#请求频率限制)


**4/24/19**: 

- 删除 [完整的撮合引擎数据(Level&nbsp;3)](#完整的撮合引擎数据(Level&nbsp;3)) 为了保护隐藏订单信息，在**received** 消息中删除 **size**  和 **funds** 字段，私人频道字段保持不变。
- 删除 [完整的撮合引擎数据(Level&nbsp;3)](#完整的撮合引擎数据(Level&nbsp;3)) 为了保护隐藏订单信息，在 **open** 消息中删除 **remainSize** 字段，私人频道字段保持不变。
- 新增 [获取所有子账号信息](#获取所有子账号信息)接口。
- 新增 [获取单个子账户信息](#获取所有子账户信息)接口。
- 新增 [获取所有子账户信息](#获取所有子账户信息)接口。
- 新增 [子母账号资金划转](#子母账号资金划转)接口。

**3/27/19** : 

- 添加 [交易对列表](#交易对列表) **feeCurrency** 字段。

**3/25/19** : 

- 添加 [全局行情快照](#全局行情快照) **volValue** 字段。
- 添加 [完整的撮合引擎数据(Level&nbsp;3)](#完整的撮合引擎数据(Level&nbsp;3)) **received**消息中  **clientOid** 字段，您可以通过 **clientOid** 来筛选订单信息。
- 添加 私人频道[余额变更](#余额变更事件) **accountId** 字段，您可以通过 **accountId** 来跟踪账户余额变更信息。
...
- 添加 [时间戳](#时间戳)订单系统和撮合引擎时间单位为  **nanoseconds**。

**2/28/19** : 

- 修改 [交易对列表](#交易对列表) 返回值描述。
- 新增 [获取V1历史充值列表](#获取V1历史充值列表)接口。
- 新增 [获取V1历史提现列表](#获取V1历史提现列表)接口。
- 新增 [获取V1历史订单列表](#获取V1历史订单列表)接口。
- 添加 部分API JSON 字段描述。
- 删除 [撮合执行数据](#撮合执行数据) 删除 **sn** 字段 。 
- 修改 [法币换算价格](#法币换算价格) 参数描述。
- 添加 创建Websocket连接时 **acceptUsermessage** 字段的描述。

**2/22/19** : 

- 添加 [24小时统计接口](#24小时统计接口) 添加**volValue**字段。

**2/21/19** : 

- 新增 [全部买卖盘(价格合并)](#全部买卖盘(价格合并)) 
- 添加 Level-1,2,3 数据添加 **time** 字段
- 新增 [法币换算价格](#法币换算价格)接口。

**2/20/19** : 

- 添加 [全局行情快照](#全局行情快照)和[行情快照](#行情快照)添加 **time** 字段
...
- 添加 公共频道添加[全部交易对瞬时行情](#全部交易对瞬时行情) 订阅
- 修改 [权限](#权限)描述
...
- 添加 [交易对行情快照](#交易对行情快照)
- 添加 [按市场的交易对行情快照](#按市场的交易对行情快照)
- 添加 官方的 [Java & Go SDK](#客户端开发库)
...
我们通过 **API-KEY** 限制 REST API： **每分钟1800次请求**，一个账号最多可以[创建](#创建API-KEY)10个API-KEY。

####硬性限制
[获取成交记录](#获取成交记录): **每十秒100次请求**（超过会被禁用10秒）
[获取订单列表](#获取订单列表): **每十秒200次请求**（超过会被禁用10秒）
...
比如，"[账户列表](#账户列表)"，默认的端点是 **/api/v1/accounts**。
...
429 | Too Many Requests -- 超过[请求频率限制](#请求频率限制)
...
代码 | 意义
---------- | -------
400001 | Any of KC-API-KEY, KC-API-SIGN, KC-API-TIMESTAMP, KC-API-PASSPHRASE is missing in your request header -- 请求头中缺少[验签参数](#创建请求)
...
当http状态代码为200时，如果业务失败，则会发生错误。您可以通过返回体中的code，在[系统错误](#系统错误码)找到特定的业务错误代码，来判断请求错误的原因。
...
储蓄账户（main）会在第一次划转资产时创建，如果未划转过资产则不会有储蓄账户。如果账户不存在，需要[创建账户](#创建账户)</aside>

请在交易之前请先[充值](#充值)到储蓄账户，再使用[内部资金划转](#内部资金划转)划转将资金从储蓄账户划转到交易账户。
...
储蓄账户和交易账户资金划转请参考[内部资金划转](#内部资金划转)。
...
请求参数 | 类型 | 是否必传 | 描述
--------- | ------- | ------- | ------- 
accountId | String | 是 | 路径参数，[账户ID](#账户列表)
...
此接口返回账户的出入账流水记录。
返回值是[分页](#分页)后的数据，根据时间降序排序，您可以获取到最新的数据。
...
请求参数 | 类型 | 是否必传 | 描述
--------- | ------- | ------- | -------  
subUserId | String | 是 |路径参数，[子账号的用户Id](#获取所有子账号信息)
...
currency | String |是 |[币种代码](#币种列表)
amount | String | 是 |转账金额，精度为[币种精度](#币种列表)正整数倍
direction | String | 是 |OUT — 母账号转子账号;IN — 子账号转母账号
subUserId | String | 是 |[子账号的用户Id](#获取所有子账号信息)
...
- 首先，按需求[创建](#创建账户)一个储蓄账户或交易账户；
- 其次，[获取accountId](#账户列表)，记录返回的Id（即accountId)；
- 最后，调用[内部资金划转](#内部资金划转)接口，发起转账请求；
...
--------- | ------- | ------- | ------- 
clientOid | String | 是 |Client Order Id，客户端创建的唯一标识，建议使用UUID
payAccountId | String | 是 |付款方的accountId [账户ID](#账户列表)
recAccountId | String | 是 |收款方的accountId [账户ID](#账户列表)
amount | String | 是 |转账金额，精度为[币种精度](#币种列表)正整数倍
...
此接口，可获取某一币种的充值地址，如果返回数据为空，请先[申请充值地址](#申请充值地址)。
...
--------- | ------- 
address | 充值地址
memo | 地址标签memo(tag)，如果返回为空，则该币种没有memo。对于没有memo的币种，在[提现](#申请提现)的时候不可以传递memo
...
memo   | String | 否 | 提现[地址标识](#获取充值地址)，没有memo(tag)的币种，不可传这个字段。
...
请求参数 | 类型 | 是否必传 | 描述
--------- | ------- | ------- | -------
withdrawalId | String | 是 |路径参数，[提现Id](#获取提现) 唯一标识
...
在下单前，请确保您的[交易账户](#账户列表)有足够的资金。一旦下单成功，您下单的金额会被冻结。[冻结金额](#HOLDS)的多少取决于您下单的类型和具体的请求参数。

请悉知，当您的订单进入买卖盘，系统会提前扣除[订单的手续费](#成交记录)。

在下单之前，请充分了解每一个[交易对](#交易对列表)的参数的意义。
...
symbol    | String | 是 | [交易对](#交易对列表) 比如，ETH-BTC                    
type      | String | 否 | 订单类型 **[limit](#限价单额外所需请求参数)** 和  **market** (默认为 **limit**)
...
stp       | String | 否 | [自成交保护](#自成交保护（STP）)（self trade prevention）**CN**, **CO**, **CB** , **DC**
...
最新交易价格是指最新成交价格，这个价格可以在最新[撮合执行数据](#撮合执行数据)里面获取。注意，由于Websocket消息有丢失的可能，无法接收所有撮合执行数据。

当您下一个止损单时，系统会提前冻结您的交易账户资金。所以，**当您下一个止损市价单时，希望您能指定具体的下单金额**。

###价格(Price)
下限价单时，price 必须以交易对的[价格增量 priceIncrement](#交易对列表)为基准，价格增量是交易对的价格的精度。比如，对BTC-USDT 这个交易对, 它的 priceIncrement 为0.00001000，那么你下单的 price 不可以小于0.00001000。您下单的价格必须为 priceIncrement 的正整数倍，否则下单时会报错，invalid prioceIncrement。

###数量(Size)
下限价单时，size 是指交易对的交易对象(即写在靠前部分的资产名)的数量。size 必须以交易对中的[数量增量 baseIncrement](#交易对列表)为基准，数量增量是交易对的数量的精度。下单的 size 为 baseIncrement 的正整数倍并且必须在 baseMinSize 和 baseMaxSize 之间。

###资金(Funds)
下市价单时，funds 字段是指交易对的定价资产(即写在靠后部分资产名)的资金。funds 必须以交易对中的[资金增量quoteIncrement](#交易对列表)为基准，资金增量是交易对的资金的精度。下单的 funds 为 quoteIncrement 的正整数倍且必须在 quoteMinSize 和 quoteMaxSize 之间。
...
请求参数 | 类型 | 是否必传 | 描述
--------- | ------- | ------- | -------
orderId | String | 是 |路径参数，[订单Id](#查询订单列表) 唯一标识
...
请求参数 | 类型 | 是否必传 | 描述
--------- | ------- | ------- | ---------
symbol | String | 否 | 取消指定[交易对](#交易对列表)的open订单
...
symbol |String| 否 | 只返回指定[交易对](#交易对列表)的订单信息
...
请求参数 | 类型 | 是否必传 | 描述
--------- | ------- | ------- | -------
orderId | String |否 | 查询该[订单Id](#查询订单列表) 的成交明细（如果指定了orderId，请忽略其他查询条件）
symbol | String |否| 查询指定[交易对](#交易对列表)的成交明细
side | String | 否 | **buy（买）** 或 **sell（卖）** 
type | String | 否 | 订单类型: **limit（限价单）**, **market(市价单)**, **limit_stop(限价止盈止损单)**, **market_stop（市价止盈止损单）** 
startAt | long | 否 |  开始时间，[Unix 时间](http://en.wikipedia.org/wiki/Unix_time)以秒为单位，只会返回在开始时间之后创建的数据。
endAt | long | 否 |  结束时间，[Unix 时间](http://en.wikipedia.org/wiki/Unix_time)以秒为单位，只会返回在结束时间以前创建的数据。
...
此接口，可获取交易对列表。如果您想获取更多交易对的市场信息，可在[全局行情快照](#全局行情快照中获取)。
...
请求参数 | 类型 | 是否必传 | 描述
--------- | ------- | ------- | -------
market | String | 否 | [交易市场](#交易市场列表): BTC, ETH, KCS, SC, NEO
...
维护一个本地的实时买卖盘，建议使用[Websocket](#Level-2市场行情)。
...
维护一个本地的实时买卖盘，建议使用[Websocket](#完整的撮合引擎数据(Level&nbsp;3))。
...
请求参数 | 类型 | 是否必传 | 描述
--------- | ------- | ------- | -------
currency | String | 是 | 路径参数，[币种标识](#币种列表)
...
```
订阅此topic可获取指定[交易对](#交易对列表)的BBO(最佳买一和卖一)数据的推送，每隔**一秒**推送一次。
此topic对带宽要求低。
...
订阅此topic对可以获取单个[交易对](#交易对列表)的行情快照信息，每隔**两秒**推送一次。
...
订阅此topic可获取指定[交易市场](#交易市场列表)的所有交易对的行情快照，每隔**2秒**推送一次。
...
订阅此topic可获取指定[交易对](#交易对列表)Level-2的买卖盘数据，每隔**2秒**推送一次。
...
如果您在维护一个本地Level-3买卖盘过程中，有不理解的地方，您可以参考用 Go Language 写的[demo](#客户端开发库)，里面包含了不同type信息的处理逻辑。
