...
The last trade price is the last price at which an order was filled. This price can be found in the latest [match message] (#c7f054198c). Note that not all match messages can be received due to dropped messages.



When placing a stop loss order, the system will not freeze the assets in your account for the order.
...
The maximum number of stop orders available for a single trading pair in one account is **20**.
...
|  Param  |  Type   |  Description                                                      |
| --------- | ------ | ------------------------------------------------------------ |
| clientOid | String | Client Order Id, Unique order id created by users to identify their orders, e.g. UUID.         |
| side      | String | buy or sell                           |
| symbol    | String | [a valid trading symbol code] (#a17b4e2866).  e.g. ETH-BTC |
| type      | String | [Optional] **limit** or **market** (set as **limit** by default)  |
| remark    | String | [Optional] remark for the order, length cannot exceed 100 utf8 characters  |             
| stop      | String | [Optional] Either **loss** or **entry**. Set as **loss** by defult|
| stopPrice | String | Stop price                      |
| stp       | String | [Optional] [self trade prevention](#80920cd667), **CN**, **CO**, **CB** or **DC**. **Do not support DC when type is market** |
| tradeType | String | [Optional] The type of trading : **TRADE** (Spot Trade), **MARGIN_TRADE** (Margin Trade). Set as **TRADE** by default |

#### **limit** LIMIT ORDER PARAMETERS
...
#### **market** MARKET ORDER PARAMETERS
...
| Field                                | Description   |
| ----------------------------------- | ------ |
| orderId                             | The ID of the order. A successful order will be assigned an order ID.  |
...
You will receive cancelledOrderIds field once the system has received the cancellation request. To know if the request is processed (successfully or not), you may check the order status or the update message from the pushes.
...
|  Param |  Type  |   Description                                                     |
| -------- | ------ | ------------------------------------------------------------ |
| orderId  | String | Path parameter, [Order ID] (#23e02e24af), unique ID of the order.|
| symbol   | String | Path parameter, indicating trading pairs                         |
...
Request via this endpoint to cancel all active stop orders. The response is a list of ids of the canceled orders.
...
|  Param |  Type  |  Description                                                        |
| --------- | ------ | ------------------------------------------------------------ |
| symbol    | String | [Optional] cancel open orders of the designated [trading pair] (#a17b4e2866)|
| tradeType | String | [Optional] cancel open orders of the designated trading types (default is to cancel TRADE (spot trade) orders)      |
| orderIds  | String | [Optional] designated order ID. You can specify multiple IDs and separate them with commas |
...
|  Param |  Type  |  Description                               |
| -------- | ------ | ------------------------------------------------------------ |
| orderId  | String | [Order ID] (#23e02e24af), unique identifier of an order, obtained via the List orders. |
| symbol   | String | Path parameter, indicating trading pairs                                |
...
Request via this endpoint to get stop order list. See the [Pagination] section (#88b6b4f79a) for retrieving additional entries after the first page.
...
|    Param |   Type |    Description                                          |
| ----------- | ------ | ------------------------------------------------------------ |
| symbol      | String | [Optional] Only list orders for a specific [symbol] (#a17b4e2866). |
| side        | String | [Optional] **buy** or **sell**                       |
| type        | String | [Optional] **limit**, **market**, **limit_stop** or **market_stop** |
...
| orderIds    | String | [Optional] List of order IDs, separated by commas   |
...
|     Field        |       Description                                          |
| --------------- | --------------------------------------------------- |
| id              | Order ID，unique identifier of an order.                               |
| symbol          | symbol                                              |
| userId          | User ID                                              |
| type            | Order type                                            |
| side            | buy or sell                                              |
...
| size            | order quantity                                            |
| funds           | order funds                                            |
| stp             | self trade prevention, inldude CN, CO, DC, CB                                          |
| timeInForce     | time InForce, include GTC, GTT, IOC, FOK                                        |
...
1. When a stop-limit order is received by the system, you will receive a ‘open’ message which means that this order entered the stop queue and waited to be triggered.  

2. When a stop-limit order is triggered, you will receive a 'triggered' message which means that this order has been triggered.

3. When a stop-limit order is cancelled, you will receive a 'cancel' message which means that this order has been cancelled.
